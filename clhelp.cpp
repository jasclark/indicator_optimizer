...
void compile_ocl_program(std::map<std::string, cl_kernel> &kernels, 
			 cl_vars_t &cv, std::string* cl_src,//const char ** cl_src,
                         int num_kerns,
			 std::list<std::string> knames)
{
  cl_int err;
  const char* sources[num_kerns];
  for (int i = 0; i < num_kerns; i++)
  {
      sources[i] = cl_src[i].c_str();
  }
  cv.main_program = clCreateProgramWithSource(cv.context, num_kerns, sources, 
					      NULL, &err);
  CHK_ERR(err);
...
void readFile(std::string* fileNames, std::string* out, int num_kerns)
{
  for (int i = 0; i < num_kerns; i++) 
  {
    ...
    if(in)
      {
        ...
        out[i].resize(in.tellg());
        in.seekg(0, std::ios::beg);
        in.read(&out[i][0], out[i].size());
        in.close();
      }
    else
      {
